...
const HOPR_STAKING_SUBGRAPH_URL = 'https://api.thegraph.com/subgraphs/name/hoprnet/staking-season2';
...
async function stakingSubgraphQuery(addresses, blockNumber, snapshot) {
    const query = {
        accounts: {
            __args: {
                first: LIMIT,
...
async function strategy(space, network, provider, addresses, options, snapshot) {
    ...
    const returnedFromSubgraph = await Promise.all(addressSubsets.map((subset) => stakingSubgraphQuery(subset, subgraphBlock, snapshot)));
