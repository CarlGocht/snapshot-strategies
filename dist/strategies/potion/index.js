...
const SUBGRAPH_URL = 'https://api.thegraph.com/subgraphs/name/dapp-testing/potion-unlock';
const abi = [
...
async function strategy(space, network, provider, addresses, options, snapshot) {
    ...
    rarityCount.forEach((rarity) => multi.call(rarity, options.address, 'rarityConfig', [rarity]));
    const configs = parseConfigs(await multi.execute());
    const pages = [...Array(6).keys()];
    const params = Object.fromEntries(pages.map((page, i) => [
        `_${page}`,
        {
            __aliasFor: 'nfts',
            __args: {
    ...
    let result = await (0, utils_1.subgraphRequest)(SUBGRAPH_URL, params);
    result = Object.values(result).flat();
    const scores = {};
    addresses.forEach((address) => (scores[address] = 0));
    result.forEach((nft) => {
        const owner = (0, address_1.getAddress)(nft.owner);
