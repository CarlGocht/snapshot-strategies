...
async function strategy(_space, _network, _provider, addresses, options) {
    ...
    const poolData = await (0, utils_1.subgraphRequest)(STAKE_GRAPH, voltDataparams);
    const subgraphData = await (0, utils_1.subgraphRequest)(UNI_GRAPH, subgraphDataParams);
    ...
    let results = {};
    results = await (0, pagination_1.strategy)(_space, network, _provider, addresses, {
        limit: 200,
        address: voltAddress,
        strategy: {
    ...
    return Object.fromEntries(addresses.map((address) => {
        ...
        if (poolData && poolData.users.length) {
            const user = poolData.users.find((r) => r.id.toLowerCase() === address.toLowerCase());
                ...
                user.vaults.forEach((v) => {
                    v.locks
                        ...
                        userCurrentStakeInVolt =
                            userCurrentStakeInVolt +
                                Number((0, units_1.formatUnits)(vlock.amount, tokenDecimals));
                    });
                    v.locks
                        ...
                        userCurrentStakeInLP =
                            userCurrentStakeInLP +
                                Number((0, units_1.formatUnits)(lplock.amount, tokenDecimals));
                    });
