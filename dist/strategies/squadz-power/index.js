...
async function strategy(space, network, provider, addresses, options, snapshot) {
    ...
    const response = await (0, utils_1.multicall)(network, provider, engineAbi, addresses.map((member) => [
        engineAddress,
        'getMemberInfo',
        [options.collectionAddress, forkNumber, member]
    ]), { blockTag });
    return Object.fromEntries(response.map((value, i) => [addresses[i], parseInt(value[5])]));
}
