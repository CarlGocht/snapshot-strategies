...
async function strategy(space, network, provider, addresses, options, snapshot) {
    ...
    const multi2 = new utils_1.Multicaller(network, provider, vault_abi, { blockTag });
    const multi3 = new utils_1.Multicaller(network, provider, dividend_abi, { blockTag });
    ...
    return Object.fromEntries(addresses.map((address) => [
        address,
        // GLIDE in wallet
        parseFloat((0, units_1.formatUnits)(result1.glide[address], 18)) +
            // GLIDE-ELA LP in farm
            parseFloat((0, units_1.formatUnits)(result1.lp1InFarm[address][0]
            ...
            parseFloat((0, units_1.formatUnits)(result1.lp2InFarm[address][0]
                .mul(result1.lp2.glide)
                .div(result1.lp2.totalSupply)
                .mul(options.lp.numerator)
                .div(options.lp.denominator), 18)) +
            // GLIDE-ELA LP in wallet
            parseFloat((0, units_1.formatUnits)(result1.lp1[address]
                .mul(result1.lp1.glide)
                .div(result1.lp1.totalSupply)
                .mul(options.lp.numerator)
                .div(options.lp.denominator), 18)) +
            // GLIDE-USDC LP in wallet
            parseFloat((0, units_1.formatUnits)(result1.lp2[address]
                .mul(result1.lp2.glide)
                .div(result1.lp2.totalSupply)
                .mul(options.lp.numerator)
                .div(options.lp.denominator), 18)) +
            // GLIDE in manual staking
            parseFloat((0, units_1.formatUnits)(result1.manualStaking[address][0], 18)) +
            // GLIDE in auto staking
            parseFloat((0, units_1.formatUnits)(result2.autoStaking[address][0]
                .mul(result2.autoStaking.shares)
                .div(precision))) +
            // GLIDE in fee staking
            parseFloat((0, units_1.formatUnits)(result3.feeStaking[address][0], 18))
    ]));
}
