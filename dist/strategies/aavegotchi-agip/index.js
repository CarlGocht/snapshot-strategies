...
async function strategy(_space, network, provider, addresses, options, snapshot) {
    ...
    addresses.map((addr) => {
        multi.call(`${options.tokenAddress}.${addr.toLowerCase()}.itemBalances`, options.tokenAddress, 'itemBalances', [addr]);
    });
    ...
    const subgraphRaw = await (0, utils_2.subgraphRequest)(AAVEGOTCHI_SUBGRAPH_URL[137], query);
    const result = Object.fromEntries(subgraphRaw.users.map((item) => {
        const ownedEntries = Object.entries(item)
            .map(([key, value]) => {
            if (key.startsWith('gotchis'))
                return value;
            else
                return [];
        })
    ...
    return Object.fromEntries(addresses.map((address) => {
        ...
        if (allGotchiInfo?.length > 0) {
            gotchisBrsEquipValue = allGotchiInfo.reduce((total, { baseRarityScore, equippedWearables }) => total +
                Number(baseRarityScore) +
                equippedWearables.reduce((currentValue, nextIter) => currentValue + prices[nextIter], 0), 0);
        }
        let ownerItemValue = 0;
        const ownerItemInfo = multiRes[options.tokenAddress][lowercaseAddr]['itemBalances'];
