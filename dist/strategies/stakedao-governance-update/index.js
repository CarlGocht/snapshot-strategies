...
async function strategy(space, network, provider, addresses, options, snapshot) {
    ...
    // *** Query *** //
    // Ethereum Side
    const SDT_Query_ETH = addresses.map((address) => [
        options.SDT_ETHEREUM,
        'balanceOf',
        [address]
    ]);
    const SDT_Locked_Query_ETH = addresses.map((address) => [
        options.veSDT_ETHEREUM,
        'locked',
        [address]
    ]);
    const veSDT_Query_ETH = addresses.map((address) => [
        options.veSDT_ETHEREUM,
        'balanceOf',
        [address]
    ]);
    const SDT_RARI_Query_ETH = addresses.map((address) => [
        options.SDT_RARI_ETHEREUM,
        'balanceOf',
        [address]
    ]);
    const xSDT_RARI_Query_ETH = addresses.map((address) => [
        options.xSDT_RARI_ETHEREUM,
        'balanceOf',
        [address]
    ]);
    // Polygon Side
    const SDT_Query_POLYGON = addresses.map((address) => [
        options.SDT_POLYGON,
        'balanceOf',
        [address]
    ]);
    // *** Response *** //
    const responseETH = await (0, utils_1.multicall)(options.network_ETHEREUM, (0, utils_2.getProvider)(options.network_ETHEREUM), abi, [
        ...SDT_Query_ETH,
    ...
    const responsePOLYGON = await (0, utils_1.multicall)(options.network_POLYGON, (0, utils_2.getProvider)(options.network_POLYGON), abi, [...SDT_Query_POLYGON], {
        blockTag
    });
