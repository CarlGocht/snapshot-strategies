...
async function strategy(space, network, provider, addresses, options, snapshot) {
    ...
    // Query
    const veSDTBalanceQuery = addresses.map((address) => [
        veSDT,
        'balanceOf',
        [address]
    ]);
    const sdTokenBalanceQuery = addresses.map((address) => [
        options.gauge,
        'balanceOf',
        [address]
    ]);
    const responseBlockRef = await (0, utils_1.multicall)(network, provider, abi, [
    ...
    const responseOtherBlock = [];
    for (let i = options.sampleStep - 1; i > 0; i--) {
        blockTag = blockTagList[i - 1];
        responseOtherBlock.push(await (0, utils_1.multicall)(network, provider, abi, [...sdTokenBalanceQuery], {
            blockTag
        }));
    }
    // Constant
    ...
    const adjustedBalance = Object.fromEntries(Array(addresses.length)
        .fill('x')
        .map((_, i) => {
        ...
        const derived_i = avgSdTokenBalance * F1 + gaugeSupply * veSDTRatio * F2;
