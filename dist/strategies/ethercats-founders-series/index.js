...
async function strategy(space, network, provider, addresses, options, snapshot) {
    ...
    const response = await (0, utils_1.subgraphRequest)(url[network], request);
    const scores = {};
    addresses.forEach((address) => {
        const score = response.erc1155Balances?.length > 0
            ? response.erc1155Balances
                .filter((x) => x.account.id == address.toLowerCase())
                .reduce((acc, account) => {
                const score = getPower(account?.token?.identifier || '0', account?.valueExact || '0');
                return acc + score;
            }, 0)
            : 0;
