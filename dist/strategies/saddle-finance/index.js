...
async function strategy(space, network, provider, addresses, options, snapshot) {
    ...
    const vestingAddrsBalanceRes = (0, utils_1.multicall)(network, provider, abi, vestingContractAddrs_1.vestingContractAddrs.map((vestingContractAddress) => [
        SDLTokenAddress,
        'balanceOf',
    ...
    const mappedBeneficiariesToVestingContract = balances[2].reduce((acc, addr, i) => ({
        ...acc,
    ...
    for (const [address, amount] of Object.entries(mappedBeneficiariesToVestingContract)) {
        const addr = address.toLowerCase();
        if (userTotal[addr])
            userTotal[addr] += amount;
        else
