...
async function strategy(space, network, provider, addresses, options, snapshot) {
    ...
    do {
        const response = await (0, utils_1.subgraphRequest)(options.graph, makeQuery(snapshot, options.minter_account_id, options.tokens, skip, blacklisted_account_ids, blacklisted_nft_ids));
    ...
    const scores = Object.fromEntries(addresses.map((address) => [address, balances[address.toLowerCase()]]));
    return scores;
}
