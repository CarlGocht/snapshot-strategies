...
async function strategy(space, network, provider, addresses, options, snapshot) {
    ...
    if (!options.vestingAddresses || options.vestingAddresses.length === 0) {
        throw new Error('Invalid options provided! Please make sure vestingAddresses is provided.');
    }
    if (options.vestingAddresses.length > maxVestingAddressCount) {
        throw new Error(`Too many vesting addresses provided! The maximum allowed is ${maxVestingAddressCount}.`);
    }
    options.vestingAddresses.forEach((vestingAddress) => {
        if (!('address' in vestingAddress) ||
            ...
            throw new Error('Invalid options provided! Please make sure each vesting address object has all required properties.');
        }
    });
    if (options.clampPercentage < 0 || options.clampPercentage > 1) {
        throw new Error('Invalid clamp percentage! Please provide a number between 0 and 1.');
    }
    if (!options.votingEscrowContractAddress || !options.decimals) {
        throw new Error('Invalid options provided! Please make sure votingEscrowContractAddress and decimals are provided.');
    }
    ...
    const vestingAddressesArray = [];
    options.vestingAddresses.forEach((vestingAddress) => {
        const { address, lockedTokens, cliffMonths, vestingMonths, startDateTimestamp, initialReleasePercentage } = vestingAddress;
        // Calculate the cliffEndDateTimestamp by adding the cliff duration (in seconds) to the startDateTimestamp.
        const cliffEndDateTimestamp = startDateTimestamp + ((cliffMonths * 365) / 12) * (24 * 60 * 60);
        // Calculate the endDateTimestamp by adding the total vesting duration (in seconds) to the startDateTimestamp.
        const endDateTimestamp = startDateTimestamp +
            (((cliffMonths + vestingMonths) * 365) / 12) * (24 * 60 * 60);
        const votingPower = votingPowerCalc(lockedTokens, now, startDateTimestamp, cliffEndDateTimestamp, endDateTimestamp, initialReleasePercentage);
    ...
    Object.values(vestingVotingPower).forEach((value) => {
        if (typeof value === 'number')
            vestingTotalVotingPower += value;
    });
    // Get voting escrow's total voting power
    const votingEscrowContract = new contracts_1.Contract(options.votingEscrowContractAddress, abi, provider);
    const votingEscrowTotalVotingPower = await votingEscrowContract.totalSupply(now);
    const escrowTotalVotingPower = parseFloat((0, units_1.formatUnits)(votingEscrowTotalVotingPower, options.decimals));
    ...
    const vestingClampedTotalVotingPower = calculateClampedVotingPower(escrowTotalVotingPower, options.clampPercentage);
 ...
 */
function votingPowerCalc(lockedToken, currentDateTS, startDateTS, cliffEndDateTS, endDateTS, initialReleasePercentage) {
    ...
    if (endTimestamp < currentTimestamp)
        return 0; // If endTimestamp is in the past, return 0 voting power
    if (startTimestamp > endTimestamp)
        throw new Error('Vesting start date cannot begin after the end date.');
    if (startTimestamp > cliffEndTimestamp)
        throw new Error('Vesting start date cannot begin after the cliff end date.');
    ...
    if (cliffDuration + vestingDuration != totalDuration)
        throw new Error("Cliff duration and vesting duration don't match up to total duration.");
    // Calculate remaining cliff duration and vesting duration
    let remainingCliffDuration;
    if (currentTimestamp >= cliffEndTimestamp)
        remainingCliffDuration = 0;
    else
        remainingCliffDuration = cliffEndTimestamp - currentTimestamp;
    let remainingVestingDuration;
    if (currentTimestamp >= cliffEndTimestamp)
        remainingVestingDuration = endTimestamp - currentTimestamp;
    else
        remainingVestingDuration = vestingDuration;
    // Clamp remaining cliff duration and vesting duration to maxLockupDuration and maxVestingDuration respectively
    const clampedRemainingCliffDuration = Math.min(remainingCliffDuration, maxLockupDuration);
    const clampedRemainingVestingDuration = Math.min(remainingVestingDuration, maxVestingDuration);
    // Calculate decayed voting power for the cliffDuration (25% portion)
    const decayCliffVPInitialRelease = (initialReleaseAmount / maxLockupDuration) * clampedRemainingCliffDuration;
    // Calculate decayed voting power for the cliffDuration (75% portion)
    const decayCliffVPAdjustedLockedToken = (adjustedLockedTokenAmount / maxLockupDuration) *
    ...
    const decayVestingVPAdjustedLockedToken = (remainingTokenAmount / maxVestingDuration) *
        clampedRemainingVestingDuration *
        0.5; // Average the decayed voting power
    // Combine all decayed voting power
    const currentVotingPower = decayCliffVPInitialRelease +
 ...
 */
function calculateClampedVotingPower(escrowVotingPower, vestingClampPercentage) {
