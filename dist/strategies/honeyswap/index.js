...
async function getPairs(network, snapshot, token) {
    ...
    const [result1, result2] = await Promise.all(['token0', 'token1'].map((prop) => (0, utils_1.subgraphRequest)(HONEYSWAP_SUBGRAPH_URL[network].exchange, getParams(prop))));
...
async function getPools(network, snapshot, token) {
    ...
    const result = await (0, utils_1.subgraphRequest)(HONEYSWAP_SUBGRAPH_URL[network].farm, params);
    return Object.fromEntries(result.pools.map((pool) => [pool.id, pairs[pool.id]]));
}
async function getStakedBalances(network, snapshot, token, addresses) {
    ...
    while (true) {
        ...
        page++;
        if (result.deposits.length < PAGE_SIZE)
            break;
    }
    return deposits.map((deposit) => {
        const pool = pools[deposit.pool.id];
        const amount = parseFloat((0, units_1.formatUnits)(deposit.amount, pool.decimals)) * pool.rate;
        return {
...
async function strategy(_space, network, _provider, addresses, options, snapshot) {
    ...
    const result = await (0, utils_1.subgraphRequest)(HONEYSWAP_SUBGRAPH_URL[network].exchange, params);
    const score = {};
