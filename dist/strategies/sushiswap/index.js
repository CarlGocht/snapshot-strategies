...
const SUSHISWAP_SUBGRAPH_URL = {
    ...
    masterChef: {
        ...
        '100': 'matthewlilley/xdai-minichef',
...
async function getPairs(network, snapshot, token) {
    ...
    const requestPairs = async (tokenId) => {
        ...
        let page = 0;
        while (true) {
            const result = await (0, utils_1.subgraphRequest)(theGraph_baseUrl + SUSHISWAP_SUBGRAPH_URL.exchange[network], getParams(tokenId, page));
            pairs = pairs.concat(result.pairs);
            page++;
            if (result.pairs.length < PAGE_SIZE)
                break;
        }
        return pairs;
    };
    const [pairs1, pairs2] = await Promise.all(['token0', 'token1'].map((tokenId) => requestPairs(tokenId)));
    return Object.fromEntries(pairs1.concat(pairs2).map((pair) => {
...
async function getPools(network, snapshot, token, masterChefUrl) {
    ...
    while (true) {
        ...
        const result = await (0, utils_1.subgraphRequest)(masterChefUrl, params);
        pools = pools.concat(result.pools);
        page++;
        if (result.pools.length < PAGE_SIZE)
            break;
    }
    return Object.fromEntries(pools.map((pool) => [pool.id, pairs[pool.pair]]));
}
...
async function strategy(_space, network, _provider, addresses, options, snapshot) {
    ...
    const result = await (0, utils_1.subgraphRequest)(theGraph_baseUrl + SUSHISWAP_SUBGRAPH_URL.exchange[network], params);
    const score = {};
    ...
    if (options.useStakedBalances === 'true' &&
        SUSHISWAP_SUBGRAPH_URL.masterChef[network]) {
        const stakedBalances = await getStakedBalances(network, snapshot, options, addresses);
