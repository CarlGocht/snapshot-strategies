...
async function strategy(_space, network, provider, addresses, options, snapshot) {
    ...
    const walletQuery = addresses.map((address) => [
        options.wapGhstAddress,
        'balanceOf',
        [address]
    ]);
    ...
    let slicedWalletQueries = [walletQuery];
    ...
    let res = await (0, utils_1.multicall)(network, provider, tokenAbi, [...slicedWalletQueries[0]], { blockTag });
    if (slicedWalletQueries.length > 1) {
        const res2 = await (0, utils_1.multicall)(network, provider, tokenAbi, [...slicedWalletQueries[1]], { blockTag });
        res = [...res, ...res2];
    }
    let slicedStakeQueries = [stakeQuery];
    ...
    const res3 = await (0, utils_1.multicall)(network, provider, tokenAbi, [...slicedStakeQueries[0]], { blockTag });
    res = [...res, ...res3];
    if (slicedStakeQueries.length > 1) {
        const res4 = await (0, utils_1.multicall)(network, provider, tokenAbi, [...slicedStakeQueries[1]], { blockTag });
        res = [...res, ...res4];
    }
    const unitWapGHST_res = await (0, utils_1.multicall)(network, provider, tokenAbi, [[options.wapGhstAddress, 'convertToAssets', ['1000000000000000000']]], { blockTag });
