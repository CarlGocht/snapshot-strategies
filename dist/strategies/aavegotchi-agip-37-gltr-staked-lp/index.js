...
async function strategy(_space, network, provider, addresses, options, snapshot) {
    ...
    let slicedStakedQueries = [stakeQuery];
    ...
    let res = await (0, utils_1.multicall)(network, provider, tokenAbi, [
    ...
    if (slicedStakedQueries.length > 1) {
        const res2 = await (0, utils_1.multicall)(network, provider, tokenAbi, [...slicedStakedQueries[1]], { blockTag });
        res = [...res, ...res2];
    }
    ...
    for (let addressIndex = 0; addressIndex < addresses.length; addressIndex++) {
        ...
        const tokens = {
            staked: {
                ghstFudLp: Number(res[i]._info[options.ghstFudPoolId].userBalance.toString()) /
                    1e18,
                ghstFomoLp: Number(res[i]._info[options.ghstFomoPoolId].userBalance.toString()) /
                    1e18,
                ghstAlphaLp: Number(res[i]._info[options.ghstAlphaPoolId].userBalance.toString()) /
                    1e18,
                ghstKekLp: Number(res[i]._info[options.ghstKekPoolId].userBalance.toString()) /
                    1e18,
                ghstGltrLp: Number(res[i]._info[options.ghstGltrPoolId].userBalance.toString()) /
                    1e18,
                ghstUsdcLp: Number(res[i]._info[options.ghstUsdcPoolId].userBalance.toString()) /
                    1e18,
