...
async function strategy(_space, network, provider, addresses, options, snapshot) {
    ...
    const afterStakingUpgrade = blockTag === 'latest' || blockAfterStakingUpgrade < blockTag;
    ...
    const ghstQuickTokensPerUni = tokensPerUni(ghstQuickTokenBalanceInUni, ghstQuickTotalSupply);
    const ghstUsdcTotalSupply = res[2];
    const ghstUsdcTokenBalanceInUni = res[3];
    const ghstUsdcTokensPerUni = tokensPerUni(ghstUsdcTokenBalanceInUni, ghstUsdcTotalSupply);
    const response = res.slice(8);
    let entries;
    if (afterStakingUpgrade) {
        const ghstWethTotalSupply = res[4];
        const ghstWethTokenBalanceInUni = res[5];
        const ghstWethTokensPerUni = tokensPerUni(ghstWethTokenBalanceInUni, ghstWethTotalSupply);
        const ghstWmaticTotalSupply = res[6];
        const ghstWmaticTokenBalanceInUni = res[7];
        const ghstWmaticTokensPerUni = tokensPerUni(ghstWmaticTokenBalanceInUni, ghstWmaticTotalSupply);
        entries = response.map((userStakeInfo, i) => {
            const votePowerAmounts = userStakeInfo._staked.map((info) => {
                if (info.poolAddress.toLowerCase() === options.tokenAddress.toLowerCase()) {
                    return Number(info.amount.toString()) / 1e18;
                }
                if (info.poolAddress.toLowerCase() ===
                    options.ghstQuickAddress.toLowerCase()) {
                    return ((Number(info.amount.toString()) / 1e18) * ghstQuickTokensPerUni);
                }
                if (info.poolAddress.toLowerCase() ===
                ...
                if (info.poolAddress.toLowerCase() ===
                    ...
                    return ((Number(info.amount.toString()) / 1e18) * ghstWmaticTokensPerUni);
                }
                return 0;
            });
